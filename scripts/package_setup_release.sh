...
#
# Writes: dist/Die-Hard-Trilogy-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Die-Hard-Trilogy-Recomp \
  --exe-name Die_Hard_Trilogy_Recompiled \
  --display-name "Die Hard Trilogy Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Die Hard Trilogy disc" \
